    ...
    <form action="{{ route('irt.storeAnswers') }}" method="POST">
        ...
        @else
            ...
            <div class="col-lg-9 col-md-9 col-12 mx-auto mb-4" style="height: auto; padding: 20px; background-color: white; border-radius: 15px; box-shadow: 0px 4px 20px rgba(0, 0, 0, 0.1);">
                <div class="row px-3">
                    @php
                        // Highlight matching parts of the question text
                        $questionText = $questions['question_text'] ?? '';
                        foreach (['A', 'B', 'C', 'D'] as $option) {
                            $optionText = $questions['option_' . $option] ?? '';
                            if ($optionText && str_contains($questionText, $optionText)) {
                                $questionText = str_replace($optionText, "<u>{$optionText}</u>", $questionText);
                            }
                        }
                    @endphp
                    <h5 class="p-3" style="font-weight: bold; color: #3e6d81;">
                        No {{ $flag+1 }}. {!! $questionText !!}
                    </h5>
                    @foreach(['A', 'B', 'C', 'D'] as $option)
